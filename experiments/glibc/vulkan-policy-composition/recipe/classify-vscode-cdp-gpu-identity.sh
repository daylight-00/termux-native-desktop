...
CONTROL_OUT=${CONTROL_OUT:?set CONTROL_OUT to a completed VS Code CDP GPU identity probe}
OUT=${OUT:-$CONTROL_OUT}
...
case "$classification" in
    ...
    FREEDRENO_TURNIP)
        ...
        if [ "$provider_path_relation" = PRESENT ] && [ "$device_node_relation" = PRESENT ]; then
            correlation_state=PASS
        fi
        ;;
esac
...
if [ "$classification" = UNKNOWN ]; then
    ...
    printf 'VS Code CDP GPU identity classification: UNKNOWN\n' >&2
    printf 'evidence: %s\n' "$CONTROL_OUT" >&2
    exit 1
fi
...
if [ "$correlation_state" != PASS ]; then
    printf 'FAIL\n' >"$OUT/cdp-gpu-identity.status"
    printf 'VS Code CDP GPU identity correlation: FAIL\n' >&2
...
printf 'VS Code CDP GPU identity classification: PASS\n'
printf 'evidence: %s\n' "$CONTROL_OUT"
printf 'classification: %s\n' "$classification"
