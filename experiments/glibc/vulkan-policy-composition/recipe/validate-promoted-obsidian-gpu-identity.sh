...
CDP_OUT="$OUT/probe"
ENV_USER_DATA="$ENV_OUT/user-data"
CDP_USER_DATA="$CDP_OUT/user-data"
...
for command in git bash awk grep mkdir date readlink pgrep sort tr sed sleep tail; do
...
# Phase 1: hostile-policy environment and exact argv through the promoted
# public launcher. The isolated user-data directory prevents normal profile use.
...
# Refuse to merge the environment tree with the CDP tree.
...
# Phase 2: fresh CDP selected-device evidence and GPU process maps.
APP="$APP" \
...
CONTROL_OUT="$CDP_OUT" OUT="$CDP_OUT" APP_LABEL=Obsidian \
...
[ "$GPU_SEEN" = 1 ] && record_gate gpu_observed PASS || record_gate gpu_observed FAIL
...
for child_class in zygote renderer gpu; do
