...
def main():
 ...
 for n,r in enumerate([r for r in ledger if r['evidence_row_id'] not in accepted_eids],1):
  ...
  pri='HARFBUZZ_BOUNDED_PROVIDER_AUTHORITY' if ident in core else 'LATER_GTK_COMPOSITION_TRANCHE'
  action=('REVIEW_EXACT_HARFBUZZ_IDENTITY_CUSTOM_STEP_CONFIGURE_PATCH_SUBPACKAGE_CONCRETE_FILENAME_DRIFT_PANGO_GTK_CONSUMER_BINDING_CONFLICT_UPDATE_AND_ROLLBACK' if pri.startswith('HARFBUZZ') else 'REVIEW_EXACT_PROVIDER_IDENTITY_ADAPTATION_CONSUMER_BINDING_CONFLICT_UPDATE_AND_ROLLBACK')
 ...
 if len(gaps)!=13:raise SystemExit(f'expected 13 gaps, got {len(gaps)}')
 reviewed=sum(g['root_mapping']!='NONE_REVIEWED_ROOT' for g in gaps)
 outside=len(gaps)-reviewed
 metadata=[
 ('schema_version','1'),('decision_policy','ADR-0005'),('selected_gtk_identity_count','36'),('accepted_provider_root_count','23'),('accepted_member_count','30'),('included_member_count','29'),('deferred_member_count','1'),('unresolved_selected_identity_count','13'),('reviewed_root_gap_count',str(reviewed)),('outside_28_root_gap_count',str(outside)),('accepted_soname_collision_count','0'),('accepted_alias_collision_count','0'),('composition_decision','REVIEWED_BLOCKED_INCOMPLETE'),('target_manifest_allowed','NO'),('next_review_tranche','HARFBUZZ_BOUNDED_PROVIDER_AUTHORITY'),('authority_effect','NO_TARGET_POPULATION_MATERIALIZATION_OR_ACTIVATION')]
