...
def main():
 ...
 if len(rows)!=10:raise SystemExit(f'expected 10 accepted member rows, got {len(rows)}')
 if len({r['soname'] for r in rows})!=10:raise SystemExit('accepted SONAME collision')
 if len({r['alias_basename'] for r in rows})!=10:raise SystemExit('accepted alias collision')

 accepted_eids={'selected:0802b57eadc2dd33925a','selected:1ae5e5d5ff7893c87e25','selected:beb2bb532f0a84c2835f','selected:d73a4eb5d9d5ee688632','selected:a7e42baafca8ed4717e3','selected:325be465ce7f532f8ff1','selected:83bc985c49ec2d778e60','selected:b869b82b3c70ee88cb30','selected:26a520f7c61bdc61e17c'}
 root_map={
 ...
 core={'libgdk_pixbuf-2.0.so.0.4200.12','libgio-2.0.so.0.8400.4','libglib-2.0.so.0.8400.4','libgmodule-2.0.so.0.8400.4','libgobject-2.0.so.0.8400.4','libpng16.so.16.48.0'}
 gaps=[]
 ...
 for n,r in enumerate([r for r in ledger if r['evidence_row_id'] not in accepted_eids],1):
  ...
  pri='GDKPIXBUF_CORE_PROVIDER_TRANCHE' if ident in core else 'LATER_GTK_COMPOSITION_TRANCHE'
  action=('ACQUIRE_OR_BIND_EXACT_TERMUX_GDKPIXBUF_CANDIDATE_AND_REVIEW_GLIB_FAMILY_AND_LIBPNG_PROVIDER_AUTHORITY' if pri.startswith('GDK') else 'REVIEW_EXACT_PROVIDER_IDENTITY_ADAPTATION_CONSUMER_BINDING_CONFLICT_UPDATE_AND_ROLLBACK')
 ...
 if len(gaps)!=27:raise SystemExit(f'expected 27 gaps, got {len(gaps)}')
