...
def main():
 ...
 if len(rows)!=29:raise SystemExit(f'expected 29 accepted member rows, got {len(rows)}')
 if len({r['soname'] for r in rows})!=29:raise SystemExit('accepted SONAME collision')
 if len({r['alias_basename'] for r in rows})!=29:raise SystemExit('accepted alias collision')
 ...
 for n,r in enumerate([r for r in ledger if r['evidence_row_id'] not in accepted_eids],1):
  ...
  pri='LIBXKBCOMMON_BOUNDED_PROVIDER_AUTHORITY' if ident in core else 'LATER_GTK_COMPOSITION_TRANCHE'
  action=('REVIEW_EXACT_LIBXKBCOMMON_IDENTITY_CUSTOM_STEP_CONCRETE_FILENAME_DRIFT_GTK_XKB_CONSUMER_BINDING_CONFLICT_UPDATE_AND_ROLLBACK' if pri.startswith('LIBXKBCOMMON') else 'REVIEW_EXACT_PROVIDER_IDENTITY_ADAPTATION_CONSUMER_BINDING_CONFLICT_UPDATE_AND_ROLLBACK')
