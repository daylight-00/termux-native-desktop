...
def main():
 ...
 if len(rows)!=22:raise SystemExit(f'expected 22 accepted member rows, got {len(rows)}')
 if len({r['soname'] for r in rows})!=22:raise SystemExit('accepted SONAME collision')
 if len({r['alias_basename'] for r in rows})!=22:raise SystemExit('accepted alias collision')

 accepted_eids={'selected:0802b57eadc2dd33925a','selected:1ae5e5d5ff7893c87e25','selected:beb2bb532f0a84c2835f','selected:d73a4eb5d9d5ee688632','selected:a7e42baafca8ed4717e3','selected:325be465ce7f532f8ff1','selected:83bc985c49ec2d778e60','selected:b869b82b3c70ee88cb30','selected:26a520f7c61bdc61e17c','selected:b577593923c28a50a012','selected:1997bce83f1eb5ffef9a','selected:6052b6396205eed6dbb6','selected:6e7f73b2a1ff4758f39a','selected:d8dfbc099c8bd2072e3f','selected:4a768de6fd1891617456','selected:ff6dae6f57afefe0d2b1','selected:e57423d1cb58b1b78ba4','selected:fec77ea4c45ec1a2990d','selected:5909031aa9e67d50214b'}
 root_map={
 ...
 core={'libcloudproviders.so.0.3.6'}
 gaps=[]
 ...
 for n,r in enumerate([r for r in ledger if r['evidence_row_id'] not in accepted_eids],1):
  ...
  pri='LIBCLOUDPROVIDERS_BOUNDED_PROVIDER_AUTHORITY' if ident in core else 'LATER_GTK_COMPOSITION_TRANCHE'
  action=('REVIEW_EXACT_LIBCLOUDPROVIDERS_IDENTITY_CONFIGURATION_CONSUMER_BINDING_CONFLICT_UPDATE_AND_ROLLBACK' if pri.startswith('LIBCLOUDPROVIDERS') else 'REVIEW_EXACT_PROVIDER_IDENTITY_ADAPTATION_CONSUMER_BINDING_CONFLICT_UPDATE_AND_ROLLBACK')
 ...
 if len(gaps)!=17:raise SystemExit(f'expected 17 gaps, got {len(gaps)}')
 reviewed=sum(g['root_mapping']!='NONE_REVIEWED_ROOT' for g in gaps)
 outside=len(gaps)-reviewed
 metadata=[
 ('schema_version','1'),('decision_policy','ADR-0005'),('selected_gtk_identity_count','36'),('accepted_provider_root_count','16'),('accepted_member_count','22'),('included_member_count','21'),('deferred_member_count','1'),('unresolved_selected_identity_count','17'),('reviewed_root_gap_count',str(reviewed)),('outside_28_root_gap_count',str(outside)),('accepted_soname_collision_count','0'),('accepted_alias_collision_count','0'),('composition_decision','REVIEWED_BLOCKED_INCOMPLETE'),('target_manifest_allowed','NO'),('next_review_tranche','LIBCLOUDPROVIDERS_BOUNDED_PROVIDER_AUTHORITY'),('authority_effect','NO_TARGET_POPULATION_MATERIALIZATION_OR_ACTIVATION')]
