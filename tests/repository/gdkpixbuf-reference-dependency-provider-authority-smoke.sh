...
import sys
p=Path(sys.argv[1]); t=p.read_text(); p.write_text(t.replace('NO_COMPOSITION_TARGET_OR_ACTIVATION_EFFECT','COMPLETE_COMPOSITION_ACCEPTED',1))
PY
if bash "$TMP/tools/docs/check-gdkpixbuf-reference-dependency-provider-authority" >/dev/null 2>&1; then
  echo 'gdkpixbuf reference provider smoke: FAIL: GLib/libpng authority widening accepted' >&2
  exit 1
fi
rm -rf "$TMP"/*
git -C "$ROOT" archive HEAD | tar -x -C "$TMP"
