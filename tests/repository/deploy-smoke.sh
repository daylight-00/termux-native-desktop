...
HOME="$HOME_TEST" PREFIX="$PREFIX_TEST" bash "$REPO/tools/deploy" --dry-run >"$ROOT/dry-run.log"
[ -L "$HOME_TEST/gl/bin" ]
[ -L "$HOME_TEST/gl/build/diag" ]

# Real deployment converts legacy directory links and installs leaf links.
HOME="$HOME_TEST" PREFIX="$PREFIX_TEST" bash "$REPO/tools/deploy" >"$ROOT/deploy.log"
[ -d "$HOME_TEST/gl/bin" ] && [ ! -L "$HOME_TEST/gl/bin" ]
[ -L "$HOME_TEST/gl/bin/gl-run" ]
[ -L "$HOME_TEST/gl/bin/gl-farm" ]
[ -L "$HOME_TEST/gl/bin/obsidian" ]
[ -L "$HOME_TEST/gl/bin/obsidian-app" ]
[ -L "$HOME_TEST/.local/bin/code" ]
[ -L "$HOME_TEST/.local/bin/startxfce-x11" ]
[ -L "$HOME_TEST/gl/env" ]
[ -L "$HOME_TEST/gl/build/build-mesa.sh" ]
[ -L "$HOME_TEST/gl/build/patches/mesa" ]
[ ! -e "$HOME_TEST/gl/build/diag" ] && [ ! -L "$HOME_TEST/gl/build/diag" ]
