...
bundle-native documentation and operations control planes active; ADR 0005 claim classification complete; exact Fontconfig accepted for bounded Pango font discovery and matching; selected-provider composition incomplete with 11 gaps; Cairo review active
> **Updated:** 2026-07-18
...
- The selected-provider composition review now records 32 exact accepted members, includes 31 in the selected GTK/transitive scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies 11 unresolved selected GTK identities. The exact `libfontconfig.so.1.14.0` member is accepted only for Pango 1.54.0 font discovery, matching and pattern properties under explicit receipt-owned configuration and cache policy; package defaults, global caches, system fonts and utility/development surfaces remain separate. The Class D composition decision remains `REVIEWED_BLOCKED_INCOMPLETE`; target-manifest generation, population and activation remain blocked while the coupled exact Cairo/Cairo-GObject provider tranche is reviewed.
