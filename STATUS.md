...
> **State:** bundle-native documentation and operations control planes active; compact constitutional authority installed; ADR 0005 claim classification complete; bounded libjpeg.so.62, GdkPixbuf 2.42.12, exact GLib four-member, libpng, libmount and libblkid providers accepted for fixed decode scopes; selected-provider composition incomplete with 20 gaps; libXcursor review active
> **Updated:** 2026-07-16
...
- The selected-provider composition review now records 18 exact accepted members, includes 17 in the selected GTK/transitive scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies 20 unresolved selected GTK identities. The Class D composition decision remains `REVIEWED_BLOCKED_INCOMPLETE`; target-manifest generation, population and activation remain blocked while the next exact libXcursor provider tranche is reviewed.
