...
- `docs/refactor/0076-scoped-vulkan-policy-promotion-candidate.md` — promoted source transaction and pre-deploy validation plan.
- `docs/refactor/` — full repository migration source of truth.

## Open questions

- The scoped Vulkan policy source transaction has not yet passed the real-device pre-deploy receipt, live deployment, or post-deploy GPU/CPU workload gates.
...
- [ ] run the no-mutation scoped Vulkan policy pre-deploy gate on the real Termux checkout
- [ ] deploy the scoped policy transaction only after that receipt passes
- [ ] validate live baseline sanitation, explicit profile selection, `gl-run`, VS Code GPU/CPU, and Obsidian GPU/CPU paths
