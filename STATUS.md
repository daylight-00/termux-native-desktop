...
bundle-native documentation and operations control planes active; ADR 0005 claim classification complete; exact HarfBuzz accepted for bounded Pango core OpenType shaping; selected-provider composition incomplete with 12 gaps; Fontconfig review active
> **Updated:** 2026-07-17
...
- The selected-provider composition review now records 31 exact accepted members, includes 30 in the selected GTK/transitive scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies 12 unresolved selected GTK identities. The exact `libharfbuzz.so.0.61010.0` member is accepted only for Pango 1.54.0 core OpenType shaping in the selected GTK text-layout scope; sibling and optional feature surfaces remain separate. The Class D composition decision remains `REVIEWED_BLOCKED_INCOMPLETE`; target-manifest generation, population and activation remain blocked while the next exact Fontconfig provider tranche is reviewed.
