...
- The selected-provider composition review now records 37 exact accepted members, includes 36 in the selected GTK/transitive scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies 6 unresolved selected GTK identities. Exact `libpixman-1.so.0.46.4` is accepted only as the direct pixel-compositing prerequisite of exact `libcairo.so.2.11802.2`; its generic no-SIMD recipe boundary does not claim upstream byte or performance equivalence. Exact Cairo/Cairo-GObject authority is now accepted atomically with the exact Pixman edge and complete accepted dependency closure. The Class D composition decision remains `REVIEWED_BLOCKED_INCOMPLETE`; target-manifest generation, population and activation remain blocked. Exact project-produced libXdamage now has a qualified Class B recipe, retained Class C producing record and bounded GTK 3.24.49 GDK X11 provider authority; approved-supplier publication and ordinary bionic or Debian bytes remain non-authoritative. Exact Graphite2 is accepted only for the HarfBuzz 10.1.0 Graphite shaping prerequisite. The coupled AT-SPI2/ATK family is stopped by the absence of an approved glibc package and pinned producing recipe; ordinary bionic packages and metapackages remain non-authoritative. The GTK 3.24.49 GDK/GTK core pair is also stopped by the absence of an approved glibc package and pinned producing recipe; ordinary bionic GTK3 remains non-authoritative. Exact `libselinux.so.1` is now likewise stopped: only ordinary bionic `libandroid-selinux` and Android platform boundary observations exist, with no approved glibc package or pinned producing recipe. Six selected gaps remain open: the three-member AT-SPI2/ATK family, the two-member GTK 3 core pair and libSELinux. The production-boundary decision activates atomic AT-SPI2/ATK candidate preparation next, defers GTK 3 until that lane is resolved, and withholds libSELinux build authorization pending direct-consumer necessity.
...
- Current Class distribution is 36 A, 50 B, 2 C and 3 D. The two Class C rows retain exact project-produced libjpeg and libXdamage producing records; their bounded provider claims remain separate Class B decisions.
