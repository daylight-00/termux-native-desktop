...
> **State:** bundle-native documentation and operations control planes active; compact constitutional authority installed; ADR 0005 claim classification complete; exact libthai, libdatrie and transitive libiconv accepted for bounded Pango Thai breaking; selected-provider composition incomplete with 17 gaps; libcloudproviders review active
> **Updated:** 2026-07-17
...
- The selected-provider composition review now records 22 exact accepted members, includes 21 in the selected GTK/transitive scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies 17 unresolved selected GTK identities. The exact `libXcursor.so.1.0.2` member is accepted only for GTK 3.24.49 X11 cursor theme/image/surface handling. The Class D composition decision remains `REVIEWED_BLOCKED_INCOMPLETE`; target-manifest generation, population and activation remain blocked while the next exact libcloudproviders provider tranche is reviewed.
