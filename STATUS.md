...
bundle-native documentation and operations control planes active; ADR 0005 claim classification complete; exact Pixman accepted as the Cairo pixel-compositing prerequisite; selected-provider composition incomplete with 10 gaps; Cairo review active
> **Updated:** 2026-07-18
...
- The selected-provider composition review now records 33 exact accepted members, includes 32 in the selected GTK/transitive scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies 10 unresolved selected GTK identities. Exact `libpixman-1.so.0.46.4` is accepted only as the direct pixel-compositing prerequisite of exact `libcairo.so.2.11802.2`; its generic no-SIMD recipe boundary does not claim upstream byte or performance equivalence. The Class D composition decision remains `REVIEWED_BLOCKED_INCOMPLETE`; target-manifest generation, population and activation remain blocked while the coupled exact Cairo/Cairo-GObject provider tranche is reviewed.
