...
- The selected-provider composition review now records 42 exact accepted members, includes 41 in the selected GTK/transitive scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies one unresolved selected identity: `libselinux.so.1`. The exact atomic GTK 3.24.49 `libgdk-3.so.0.2417.32` and `libgtk-3.so.0.2417.32` pair has bounded core library provider authority only. Headers, pkg-config, unversioned aliases, tools, Broadway daemon, input modules, print backends, schemas, GIR/typelib target membership, display/service execution, target population, deployment and activation remain excluded. The next task is a direct-consumer necessity and security-semantics review for libSELinux; build authorization remains withheld.
