...
> **State:** bundle-native documentation and operations control planes active; compact constitutional authority installed; ADR 0005 claim classification complete; exact FriBidi accepted for bounded Pango/GTK Unicode bidi processing; selected-provider composition incomplete with 15 gaps; FreeType review active
> **Updated:** 2026-07-17
...
- The selected-provider composition review now records 24 exact accepted members, includes 23 in the selected GTK/transitive scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies 15 unresolved selected GTK identities. The exact `libXcursor.so.1.0.2` member is accepted only for GTK 3.24.49 X11 cursor theme/image/surface handling. The Class D composition decision remains `REVIEWED_BLOCKED_INCOMPLETE`; target-manifest generation, population and activation remain blocked while the next exact FreeType provider tranche is reviewed.
