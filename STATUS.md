...
> **State:** bundle-native documentation and operations control planes active; compact constitutional authority installed; ADR 0005 claim classification complete; seven no-token recipes confirmed Class A and all seven bounded provider roots accepted; Pango filename continuity decided; exact libjpeg.so.62 and GdkPixbuf 2.42.12 providers accepted for bounded decode scopes; selected-provider composition incomplete; GdkPixbuf reference dependency review active
> **Updated:** 2026-07-16
...
- The selected-provider composition review now records 11 exact accepted members, includes 10 in the selected GTK scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies 26 unresolved selected GTK identities. The Class D composition decision remains `REVIEWED_BLOCKED_INCOMPLETE`; target-manifest generation, population and activation remain blocked while the GLib/libpng and transitive util-linux dependency providers are reviewed.
- The official-source GdkPixbuf 2.42.12 scratch build produced exact `libgdk_pixbuf-2.0.so.0.4200.12` SHA-256 `0c1404c...`, removed its build-tree search path, and passed JPEG/PNG file and memory decoding with exact scratch mappings. This exact final object now has bounded provider authority. GLib, libpng, and the observed `libmount`/`libblkid` transitive providers remain separate open decisions; tracked repository state and live glibc/provider paths were unchanged.
