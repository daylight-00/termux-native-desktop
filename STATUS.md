...
bundle-native documentation and operations control planes active; ADR 0005 claim classification complete; exact Cairo and Cairo-GObject accepted atomically; selected-provider composition incomplete with 8 gaps; libXdamage glibc-candidate blocker recorded; Graphite2 evidence acquisition active
> **Updated:** 2026-07-18
...
- The selected-provider composition review now records 35 exact accepted members, includes 34 in the selected GTK/transitive scope, defers libtasn1 until a GnuTLS/security profile is selected, finds zero accepted-member SONAME or alias collisions, and identifies 8 unresolved selected GTK identities. Exact `libpixman-1.so.0.46.4` is accepted only as the direct pixel-compositing prerequisite of exact `libcairo.so.2.11802.2`; its generic no-SIMD recipe boundary does not claim upstream byte or performance equivalence. Exact Cairo/Cairo-GObject authority is now accepted atomically with the exact Pixman edge and complete accepted dependency closure. The Class D composition decision remains `REVIEWED_BLOCKED_INCOMPLETE`; target-manifest generation, population and activation remain blocked. Exact libXdamage authority is stopped by the absence of both an approved glibc package and a pinned glibc recipe root; the ordinary bionic package is explicitly non-authoritative. Graphite2 evidence acquisition is the next independent tranche.
