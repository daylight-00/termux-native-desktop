...
The OJ-001 `libjpeg.so.62` repository-candidate disposition and pinned-source compatibility-provider production boundary are in [`libjpeg-so-62-provider-candidate-disposition.md`](libjpeg-so-62-provider-candidate-disposition.md). The first scratch-built candidate result and its blocking colon-only `DT_RUNPATH` are reviewed in [`libjpeg-so-62-compatibility-provider-candidate-result-review.md`](libjpeg-so-62-compatibility-provider-candidate-result-review.md).
