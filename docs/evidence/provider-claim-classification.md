...
provider authority accepted inside inventory: 18
...
The exact `libfribidi.so.0.4.0` provider remains accepted only for Pango 1.54.0 core Unicode bidirectional processing. Exact `libfreetype.so.6.20.2` remains accepted for the bounded Pango/GTK FT2 font-engine path, with four pre-existing conditional objects included only for its compression-feature closure. Exact `libxkbcommon.so.0.8.0` is now accepted for the bounded GTK 3.24.49 Wayland XKB keymap and keyboard translation path, with SONAME `libxkbcommon.so.0` and explicit `0.0.0` to `0.8.0` concrete-filename continuity. The next smallest phase is HarfBuzz. No target paths are generated.

## Stop line

Do not:

- treat claim classification alone as provider acceptance outside the eighteen explicit bounded provider review rows;
