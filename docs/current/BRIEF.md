...
`termux-native-desktop` turns a stock, non-root Android phone into a practical native Termux research and development workstation. The user Termux checkout remains authoritative for network-backed Git and Android/device execution. Web-chat prepares bounded artifacts and uses one Google Drive attempt per outbound artifact before the documented identical sandbox fallback.
